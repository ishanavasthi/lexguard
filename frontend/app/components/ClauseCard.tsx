  ...
  return (
    <Card className={cn("border-l-4", styles.border)}>
      <CardHeader>
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <Badge variant="secondary">{formatCategory(clause.category)}</Badge>
      ...
      <CardContent className="space-y-4">
        <p className="text-base text-slate-800">{clause.plain_english}</p>
        ...
        <div className="rounded-md border border-amber-200 bg-amber-50 p-3">
          ...
          <p className="text-sm text-amber-900">{clause.negotiation_tip}</p>
        </div>

        <Collapsible open={open} onOpenChange={setOpen}>
          <CollapsibleTrigger className="flex items-center gap-1 text-xs font-medium text-slate-500 hover:text-slate-700">
            {open ? (
              <>
